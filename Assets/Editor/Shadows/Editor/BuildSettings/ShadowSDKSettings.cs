...
public class ShadowSDKSettings : EditorWindow {
   ...
   [MenuItem("ShadowCreator/ProjectSettings")]
    static void Init() {
        if(window == null) {
            window = EditorWindow.GetWindow(typeof(ShadowSDKSettings));
    ...
    static string assetPath = "Assets/ShadowCreator/Editor/BuildSettings/Parm.asset";
    ...
    static void NoticWindow() {
        ...
        if(EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android) {
            GUILayout.Label("Notice: Recommended Project settings for ShadowSDK", styleNoticeText);
        } else {
    ...
    static void PlayerSetMust() {
        if(PlayerSettings.productName == "New Unity Project") {
            PlayerSettings.productName = "sdk301";
        }
        ...
        //if(EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android) {
            ...
            //Texture2D tex2D = AssetDatabase.LoadAssetAtPath(@"Assets/ShadowCreator/Plugins/app_icon.png", typeof(Texture2D)) as Texture2D;

            //PlatformIconKind[] icons = PlayerSettings.GetSupportedIconKindsForPlatform(BuildTargetGroup.Android);

            //PlayerSettings.SetIconsForTargetGroup(BuildTargetGroup.Android, new Texture2D[6] { tex2D, tex2D, tex2D, tex2D, tex2D, tex2D }, IconKind.Application);
            //AssetDatabase.SaveAssets();
