...
public static class NativeGallery
{
	...
	{
        string r;
		return SaveToGallery( mediaBytes, album, filenameFormatted, true, callback, out r);
	}
	...
	public static Permission SaveImageToGallery( Texture2D image, string album, string filenameFormatted,out string r, MediaSaveCallback callback = null )
	{
		if( image == null )
			throw new ArgumentException( "Parameter 'image' is null!" );
		if( filenameFormatted.EndsWith( ".jpeg" ) || filenameFormatted.EndsWith( ".jpg" ) )
        {
            return SaveToGallery( GetTextureBytes( image, true ), album, filenameFormatted, true, callback ,out  r);
        }
        else if( filenameFormatted.EndsWith( ".png" ) )
			return SaveToGallery( GetTextureBytes( image, false ), album, filenameFormatted, true, callback, out r);
		else
			return SaveToGallery( GetTextureBytes( image, false ), album, filenameFormatted + ".png", true, callback, out r);
	}

	public static Permission SaveVideoToGallery( byte[] mediaBytes, string album, string filenameFormatted, out string realPath, MediaSaveCallback callback = null )
	{
        return SaveToGallery(mediaBytes, album, filenameFormatted, false, callback, out realPath);
	}
	...
	private static Permission SaveToGallery( byte[] mediaBytes, string album, string filenameFormatted, bool isImage, MediaSaveCallback callback, out string realPath)
	{
